...
from .log import log
from .point_utils import convert_to_symmetric_point, find_symmetric_point
...
def get_lattice_points(lattice: bpy.types.Lattice) -> tuple[list[int], list[list[float]]]:
...
def is_scene_updated(scene, points: list[list[float]], selected_points: list[int]) -> bool:
    return (not hasattr(check_vertex_movement, "previous_positions")) or \
...
def apply_symmetry(scene, lattice: bpy.types.Lattice, selected_point_indexes: list[int], points: list[list[float]]) -> None:
